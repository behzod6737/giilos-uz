...
const createModalOption = (manufacturers, goingElement ) =>{
	goingElement.innerHTML = null 
	goingElement.innerHTML = ' <option  selected disabled value="0">Select a manufacturer</option>'
...
const renderProduct = (products, goingElement) => {
	...
	products.forEach(product => {
		...
		getElement('.card-list__item',template).id = id
		template.querySelector('.card-img-top').src = img
		template.querySelector('.card-title').textContent = title
		template.querySelector('.card-text').textContent = price.toLocaleString('es-ES', { style: 'currency', currency: 'UZS' });
